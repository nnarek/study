------------------------------ MODULE Session7_ex1 ------------------------------
EXTENDS Integers

CONSTANT Nset
ASSUME Nset \subseteq Int

(*********

--algorithm Add2FGSem {
   variable x = 0, sem = 1, N \in Nset ; 
   process (AB \in 1..N) 
    variable t = -1 ;
     { lock: await sem = 1;
             sem := 0 ;
       ab1: t := x ;
       ab2: x := t + 1;
       unlock: sem := 1 ;
     }
 }

*********)
\* BEGIN TRANSLATION (chksum(pcal) = "b7a4b235" /\ chksum(tla) = "16455887")
VARIABLES pc, x, sem, N, t

vars == << pc, x, sem, N, t >>

ProcSet == (1..N)

Init == (* Global variables *)
        /\ x = 0
        /\ sem = 1
        /\ N \in Nset
        (* Process AB *)
        /\ t = [self \in 1..N |-> -1]
        /\ pc = [self \in ProcSet |-> "lock"]

lock(self) == /\ pc[self] = "lock"
              /\ sem = 1
              /\ sem' = 0
              /\ pc' = [pc EXCEPT ![self] = "ab1"]
              /\ UNCHANGED << x, N, t >>

ab1(self) == /\ pc[self] = "ab1"
             /\ t' = [t EXCEPT ![self] = x]
             /\ pc' = [pc EXCEPT ![self] = "ab2"]
             /\ UNCHANGED << x, sem, N >>

ab2(self) == /\ pc[self] = "ab2"
             /\ x' = t[self] + 1
             /\ pc' = [pc EXCEPT ![self] = "unlock"]
             /\ UNCHANGED << sem, N, t >>

unlock(self) == /\ pc[self] = "unlock"
                /\ sem' = 1
                /\ pc' = [pc EXCEPT ![self] = "Done"]
                /\ UNCHANGED << x, N, t >>

AB(self) == lock(self) \/ ab1(self) \/ ab2(self) \/ unlock(self)

(* Allow infinite stuttering to prevent deadlock on termination. *)
Terminating == /\ \A self \in ProcSet: pc[self] = "Done"
               /\ UNCHANGED vars

Next == (\E self \in 1..N: AB(self))
           \/ Terminating

Spec == Init /\ [][Next]_vars

Termination == <>(\A self \in ProcSet: pc[self] = "Done")

\* END TRANSLATION 

=============================================================================
\* Modification History
\* Last modified Mon Mar 24 16:18:37 GET 2025 by developer
\* Last modified Fri Feb 12 09:46:15 PST 2021 by lamport
\* Created Fri Feb 12 09:44:05 PST 2021 by lamport
