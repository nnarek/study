...
__global__ void fnv1a_hash_kernel(unsigned int target_hash, int R, char* output_password,int numCombinations) {
...
void solve_for_given_pass_length(unsigned int target_hash, int R, char* output_password) {
    ...
    fnv1a_hash_kernel<password_length><<<blocksPerGrid,threadsPerBlock>>>(target_hash, R, output_password,numCombinations);
}
