...
Proof.
  intros b c.
  destruct b.
  all: repeat (simpl; intros H; rewrite H; reflexivity ).
Qed.
...
Definition get_modifier (g: grade) : modifier :=
  match g with
  | Grade l m => m
  end.
...
:=
  let letter_comp := letter_comparison (get_letter g1) (get_letter g2) in
  match letter_comp with
  | Eq => modifier_comparison (get_modifier g1) (get_modifier g2) 
  | _ => letter_comp
  end.
...
Proof.
  ...
  destruct g.
  destruct l.
  all: repeat (destruct m; simpl; reflexivity ).
  destruct m.
  all: repeat (simpl; reflexivity ).
  simpl.
  rewrite H.
